...
def escape_for_robot(text: str) -> str:
    ...
    Also protects consecutive spaces so Robot does not split a single argument
    into multiple arguments (Robot uses 2+ spaces as token separator).
    """
    ...
    # Protect runs of spaces so a single argument is never tokenized by Robot
    # due to "two-or-more spaces" separators. `${SPACE}` is a built-in variable.
    # Example: "a  b" -> "a${SPACE}${SPACE}b".
    escaped = re.sub(r' {2,}', lambda m: '${SPACE}' * len(m.group(0)), escaped)
    return escaped
...
class Compiler:
    ...
    def __init__(self, options: Optional[CompilerOptions] = None):
        ...
        # Filter to base64-encode values for safe Robot argument transport
        self.jinja_env.filters['b64encode'] = lambda text: base64.b64encode(
            str(text if text is not None else "").encode("utf-8")
        ).decode("ascii")
    ...
    def _generate_main_robot(self, bot_def: BotDefinition) -> str:
        ...
        all_nodes = self._flatten_nodes(bot_def.nodes)
        rendered = template.render(bot=bot_def, top_nodes=bot_def.nodes, all_nodes=all_nodes)
        return self._normalize_robot_continuations(rendered)

    @staticmethod
    def _normalize_robot_continuations(content: str) -> str:
        """
        Normaliza líneas de continuación Robot Framework que, por whitespace trimming
        de Jinja, pueden quedar concatenadas en una sola línea con múltiples `...`.
        """
        normalized_lines = []
        continuation_prefix = "...    "
        ...
        for line in content.splitlines():
            ...
            if not stripped.startswith(continuation_prefix):
                normalized_lines.append(line)
                continue

            payload = stripped[len(continuation_prefix) :]
            if continuation_prefix not in payload:
                normalized_lines.append(line)
                continue

            parts = [part.strip() for part in payload.split(continuation_prefix) if part.strip()]
            if not parts:
                normalized_lines.append(line)
                continue

            for part in parts:
                normalized_lines.append(f"{indent}{continuation_prefix}{part}")
    ...
    def _generate_manifest(self, bot_def: BotDefinition) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_level_node_count": len(bot_def.nodes),
    ...
    def _flatten_nodes(nodes):
        ...
        for node in nodes:
            ...
            if getattr(node, "children", None):
                flattened.extend(Compiler._flatten_nodes(node.children))
        return flattened
