...
class NodeDefinition(BaseModel):
    ...
    # Container nodes can embed child nodes (Studio GroupNode model)
    children: Optional[List["NodeDefinition"]] = Field(
        None, description="Nodos hijos de contenedor (control.loop/if/while/try_catch)"
    )
...
class BotDefinition(BaseModel):
    ...
    def validate_nodes(cls, v: List[NodeDefinition]) -> List[NodeDefinition]:
        ...
        # Validate unique IDs
        ids = [node.id for node in BotDefinition._iter_nodes(v)]
    ...
    def _iter_nodes(nodes: List[NodeDefinition]):
        for node in nodes:
            yield node
            if node.children:
                yield from BotDefinition._iter_nodes(node.children)

    def get_all_nodes(self) -> List[NodeDefinition]:
        """Returns a flattened list including container children."""
        return list(self._iter_nodes(self.nodes))

    def get_node(self, node_id: str) -> Optional[NodeDefinition]:
        """Gets a node by its ID (top-level or container child)."""
        for node in self._iter_nodes(self.nodes):
            if node.id == node_id:
                return node
        return None
