...
class SkuldStorage:
    ...
    def storage_write_file(
        ...
        backend = self._providers.get(provider) if provider else self._get_provider()
        payload: Union[bytes, str] = content
        if append and backend.exists(path):
            existing = backend.read(path, encoding=None)
            existing_bytes = existing if isinstance(existing, bytes) else str(existing).encode("utf-8")
            if isinstance(content, bytes):
                payload = existing_bytes + content
            else:
                payload = existing_bytes.decode("utf-8", errors="replace") + str(content)
        result = backend.write(path, payload, content_type=content_type)
    ...
    def storage_copy_file(
        ...
        backend = self._providers.get(provider) if provider else self._get_provider()
        result = backend.copy(source, destination, overwrite=overwrite)
    ...
    def storage_move_file(
        ...
        backend = self._providers.get(provider) if provider else self._get_provider()
        result = backend.move(source, destination, overwrite=overwrite)
    ...
    def generate_presigned_url(
        ...
        return backend.generate_presigned_url(path, expiration, operation)
    ...
    def create_zip_archive(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                files = backend.list(source_path, recursive=True).items
                is_directory = True
            except:
                is_directory = False
                ...
                if is_directory:
                    ...
                    for file_info in files:
                        ...
                        if file_path:
                            ...
                            content = backend.read(file_path, encoding=None)
                    ...
                    # Single file
                    content = backend.read(source_path, encoding=None)
