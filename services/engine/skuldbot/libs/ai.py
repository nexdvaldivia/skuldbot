...
Prompt Management:
    ...
    Prompts are stored as .md files in services/engine/skuldbot/ai/prompts/
...
class SkuldAI:
    ...
    def _init_client(self):
        ...
        if provider == AIProvider.OPENAI:
            ...
            try:
                from openai import OpenAI
                normalized_base_url = self._normalize_ollama_base_url(self._config.base_url)
                self._config.base_url = normalized_base_url
                self._client = OpenAI(
                    api_key="ollama",  # Ollama no requiere API key
                    base_url=normalized_base_url
                )
                logger.info(f"Ollama endpoint normalized to OpenAI-compatible base URL: {normalized_base_url}")
            except ImportError:
...
{system_prompt}"""
        ...
        while iteration < max_iterations and status == "running":
            ...
            try:
                ...
                if iteration < max_iterations:
                    # One corrective retry path: ask model to regenerate valid compact JSON.
                    ...
                    messages.append({
                        ...
                        "content": (
                            ...
                            "Regenerate a valid JSON response only, no markdown, no extra text. "
                            "If output is long, return compact JSON to avoid truncation."
                        )
                    })
                    continue
                status = "error"
    ...
    def _strip_markdown_code_fences(text: str) -> str:
        """Remueve fences markdown ```...``` conservando contenido interno."""
        value = (text or "").strip()
    ...
    def _extract_first_json_payload(text: str) -> Optional[str]:
        """
        Extrae el primer payload JSON balanceado ({...} o [...]) desde texto libre.
        Soporta strings con escapes para no romper el balanceo por llaves internas.
        """
        if not text:
            return None
        ...
        # Use the first opening brace/bracket as root candidate.
        start = -1
        root = ""
    ...
    def _sanitize_json_candidate(candidate: str) -> str:
        """Aplica normalizaciones seguras para JSON casi-válido."""
        if not candidate:
            return ""
        normalized = candidate.strip().lstrip("\ufeff")
        # Replace smart quotes often produced by copied prompts/content.
        normalized = (
            normalized.replace("“", '"')
            .replace("”", '"')
            .replace("‘", "'")
    ...
    def _parse_json_lenient(self, response: str) -> Any:
        """Intenta parsear JSON con estrategias determinísticas sin usar modelos."""
        cleaned = self._strip_markdown_code_fences(response)
        candidates: List[str] = [cleaned]

        extracted = self._extract_first_json_payload(cleaned)
        ...
        for candidate in candidates:
            variants = [candidate, self._sanitize_json_candidate(candidate)]
        ...
        if cleaned.count("{") > cleaned.count("}") or cleaned.count("[") > cleaned.count("]"):
            hint = " Likely truncated model output; reduce payload size or process 1 item per iteration."
        raise ValueError(
            f"Agent returned invalid JSON response: {errors[-1] if errors else 'unknown error'}. "
            f"Raw preview: {preview}.{hint}"
        )

    @keyword("Parse JSON Lenient")
    def parse_json_lenient(self, response: str) -> Any:
        """Parsea JSON de forma tolerante (fences, comas finales, texto envolvente)."""
        return self._parse_json_lenient(response)

    def _parse_agent_response(self, response: str) -> Dict[str, Any]:
        """
        Parsea la respuesta del agente.
        Acepta:
        - Formato ReAct esperado: {thought, action, action_input}
        - JSON de negocio directo (objeto/array): se envuelve como final_answer
        """
        data = self._parse_json_lenient(response)
        ...
        # Enterprise fallback: accept direct structured payload as final answer.
        if isinstance(data, (dict, list)):
            return {
                "thought": "Direct structured JSON response accepted as final answer.",
                "action": "final_answer",
                "action_input": {"answer": json.dumps(data, ensure_ascii=False)},
            }

        # Primitive fallback (string/number/bool/null).
        return {
            "thought": "Direct primitive response accepted as final answer.",
            "action": "final_answer",
            "action_input": {"answer": str(data)},
        }
...
{{
  ...
  "unrepairable": [
    ...
    def _normalize_ollama_base_url(base_url: Optional[str]) -> str:
        """
        Normaliza URL de Ollama para cliente OpenAI-compatible.

        OpenAI client espera endpoint base que termine en `/v1`.
        Ejemplos:
          - http://127.0.0.1:11434      -> http://127.0.0.1:11434/v1
          - http://127.0.0.1:11434/v1   -> http://127.0.0.1:11434/v1
          - http://127.0.0.1:11434/api  -> http://127.0.0.1:11434/v1
        """
        ...
        if path == "/v1":
            ...
            # Any custom path gets /v1 suffix for OpenAI-compatible route.
