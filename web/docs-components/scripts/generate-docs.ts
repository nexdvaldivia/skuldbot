...
const NODE_TEMPLATES_PATH = path.join(__dirname, '../../../apps/studio/src/data/nodeTemplates.ts');
...
function getRealisticExample(node: NodeTemplate): string {
  const examples: Record<string, Record<string, any>> = {
    ...
    // File examples
    'files.read': { source: '/data/config.json' },
    'files.write': { destination: '/output/results.json', content: '${Process Data.output}' },
