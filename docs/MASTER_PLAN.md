...
## Deuda API Conocida (pre-Sprint 1)

| Bug | Impacto | Fix |
|-----|---------|-----|
| MCP double prefix (`/api/api/v1/mcp`) | MCP endpoints inalcanzables desde frontend | `@Controller('v1/mcp')` |
| Billing/Subscription `@Controller()` vacio | Riesgo de colision de rutas | Agregar prefijos explicitos |
| Frontend llama endpoints inexistentes | marketplace/analytics, marketplace/partners CRUD | Crear endpoints o eliminar llamadas |
| Frontend `/api/subscriptions` vs backend sin prefijo | Funciona por accidente, fragil | Alinear contratos |
...
| M2.8 Support System | Stub (86 LOC) | Full ticket system: SLA, auto-assignment, merge, CSAT, notifications, business hours, escalation, canned responses, custom fields, reporting (portado de Nexion) |
...
| M7.4 Orchestrator Deployer | Luis | No iniciado | Tauri wizard + Terraform (AWS/Azure) para desplegar Orchestrators en infra del cliente. Portar de `nexion-deployer-aws/` y `nexion-deployer-azure/` |
| M7.5 Email Templates System | Luis | No iniciado | Templates sistema + marketing, rendering engine, tracking, rate limiting. Portar de Nexion `templates/email/` + `multi_email_service.py` |
...
| ID | Tarea | Asignado | Modulo |
|----|-------|:--------:|--------|
| S1-00a | Fix MCP double prefix: `@Controller('api/v1/mcp')` → `@Controller('v1/mcp')` | Luis | Bugfix |
| S1-00b | Fix Billing/Subscription empty controller prefix: add `@Controller('billing')` and `@Controller('subscriptions')`, update frontend paths | Luis | Bugfix |
| S1-00c | Fix frontend calling non-existent endpoints: `/api/marketplace/analytics`, `/api/marketplace/partners` CRUD | Luis | Bugfix |
...
### Sprint 6: Support System + Operations (2 semanas)
> Soporte a clientes con SLA, escalation y CSAT — portado de Nexion.

| ID | Tarea | Asignado | Modulo |
|----|-------|:--------:|--------|
| S6-01 | Ticket entity + TicketComment + TicketAttachment + TicketActivity | Luis | M2.8 |
| S6-02 | Support config entities (Category, Priority, Status, SLA, BusinessHours, Holiday) | Luis | M2.8 |
| S6-03 | Ticket service (CRUD, number gen, status workflow, assignment, merge, tags) | Luis | M2.8 |
| S6-04 | SLA service (policy matching, business hours calc, breach detection) | Luis | M2.8 |
| S6-05 | Ticket notification service (templates, email routing per event) | Luis | M2.8 |
| S6-06 | Auto-assignment rules engine (round-robin, team, load-balance) | Luis | M2.8 |
| S6-07 | Canned responses + custom fields per category | Luis | M2.8 |
| S6-08 | CSAT survey (create on resolve, public endpoint, quick-rate) | Luis | M2.8 |
| S6-09 | Background tasks: SLA breach check, auto-close resolved tickets | Luis | M2.8 |
| S6-10 | Support API: customer endpoints (create, list, comment, attach) | Luis | M2.8 |
| S6-11 | Support API: admin endpoints (stats, assign, merge, reports) | Luis | M2.8 |
| S6-12 | Support config API: categories, priorities, statuses, SLA, business hours, templates, rules | Luis | M2.8 |
| S6-13 | **CP UI: Support dashboard** (stats, at-risk, breached, recent) | Albert | M6.1 |
| S6-14 | **CP UI: Tickets list** (search, filters, create, export) | Albert | M6.1 |
| S6-15 | **CP UI: Ticket detail** (comments, internal notes, attachments, activity, merge, CSAT) | Albert | M6.1 |
| S6-16 | **CP UI: Support settings** (categories, SLA, business hours, templates, rules) | Albert | M6.1 |

### Sprint 7: Orchestrator UI + Client Operations (2 semanas)
...
| ID | Tarea | Asignado | Modulo |
|----|-------|:--------:|--------|
| S8-01 | Orchestrator Deployer: Tauri wizard + Terraform AWS (ECS, RDS, S3, ALB) | Luis | M7.4 |
| S8-01b | Orchestrator Deployer: Terraform Azure (App Service, PostgreSQL, Blob, Key Vault) | Luis | M7.4 |
| S8-01c | Email Templates System: system templates + marketing engine + tracking | Luis | M7.5 |
...
| S6 | Support: ticket lifecycle completo, SLA breach detectado, CSAT enviado |
| S7 | Orch UI: evidence viewer renderiza pack real, schedule builder funcional |
| S8 | >80% coverage en modulos nuevos, 0 SAST high/critical, load test pasa |
| S9 | Pentest clean, auto-update funciona, Terraform aplica sin errores |
| S10 | Catalogo normalizado 1:1, enriched catalog generado |
| S11 | Quality rules auto-fix 100% reglas universales, 0 bots sin error handling |
| S12 | ReAct loop ejecuta 4+ tools, multi-provider parity |
| S13 | RAG: top-3 templates relevantes en <500ms, >50 templates |

---

## FASE PLANNER: AI Planner Evolution (Sprints 10-13)

> Convertir el AI Planner de generador de flujos a copiloto de automatizacion.
> Referencia completa: `docs/AI_PLANNER_EVOLUTION_PLAN.md`

### Sprint 10: Catalog Normalization + Enrichment (2 semanas)
> Sin catalogo correcto, todo lo demas falla.

| ID | Tarea | Asignado | Modulo |
|----|-------|:--------:|--------|
| S10-01 | Resolver gap de 49 tipos entre nodeTemplates.ts y registry.py | Luis | Planner |
| S10-02 | Definir catalog_contract.json como source of truth canonica | Luis | Planner |
| S10-03 | Script generador: merge engine + studio → enriched catalog | Luis | Planner |
| S10-04 | CI check: falla si hay drift entre studio/engine/contract | Luis | Planner |
| S10-05 | Artefactos: enriched_full.json + enriched_by_category/*.json | Luis | Planner |
| S10-06 | Contenido editorial: patterns por nodo (cuando usar, con que combinar) | Albert | Planner |
| S10-07 | Contenido editorial: securityNotes por nodo (vault, PII, compliance) | Albert | Planner |
| S10-08 | Contenido editorial: commonCombinations por industria | Albert | Planner |
| S10-09 | Intent classifier: selecciona categorias relevantes del goal del usuario | Luis | Planner |
| S10-10 | Integrar enriched catalog en ai_generate_executable_plan (reemplaza compact) | Luis | Planner |
| S10-11 | Reescribir system prompt del planner con reglas obligatorias | Albert | Planner |

### Sprint 11: Quality Rules Engine en Rust (2 semanas)
> Cada bot generado sale production-ready automaticamente.

| ID | Tarea | Asignado | Modulo |
|----|-------|:--------:|--------|
| S11-01 | Quality Rule trait en Rust (id, severity, check, auto_fix) | Luis | Planner |
| S11-02 | Reglas universales: error handling en cada nodo | Luis | Planner |
| S11-03 | Reglas universales: secrets via vault, nunca hardcoded | Luis | Planner |
| S11-04 | Reglas universales: HTTP timeout, loop max_iterations, trigger requerido | Luis | Planner |
| S11-05 | Reglas por industria: HIPAA (PII redact, Evidence Pack) | Albert (reglas) + Luis (Rust) | Planner |
| S11-06 | Reglas por industria: SOC2 (audit log), PCI (mask cards), Insurance (evidence) | Albert (reglas) + Luis (Rust) | Planner |
| S11-07 | Auto-fix pipeline: aplica fixes automaticos antes de responder al usuario | Luis | Planner |
| S11-08 | Confidence score real basado en: template match, validation, completeness, coverage | Luis | Planner |
| S11-09 | Integracion con engine validator Python como segunda capa de validacion | Luis | Planner |
| S11-10 | **Studio UI: Validation tab** muestra violaciones con explicacion y severity | Albert | Planner |

### Sprint 12: MCP + ReAct Loop (3 semanas)
> El planner razona y usa herramientas en vez de generar a ciegas.

| ID | Tarea | Asignado | Modulo |
|----|-------|:--------:|--------|
| S12-01 | MCP tools: search_node_catalog (busca nodos por query) | Luis | Planner |
| S12-02 | MCP tools: get_node_details (schema completo de un nodo) | Luis | Planner |
| S12-03 | MCP tools: search_templates (busca bots similares en knowledge base) | Luis | Planner |
| S12-04 | MCP tools: check_compliance (verifica reglas por industria) | Luis | Planner |
| S12-05 | ReAct loop MVP en Rust: think → act → observe → repeat (OpenAI-compatible) | Luis | Planner |
| S12-06 | Multi-provider ReAct: Anthropic + Azure + Ollama parity | Luis | Planner |
| S12-07 | Hardening: audit trail de tool calls, retries, timeout, token budget | Luis | Planner |
| S12-08 | Guardrails: max iterations, max tokens, content filtering | Luis | Planner |
| S12-09 | **Studio UI: Tool calls trace** visible en Chat panel (que tools uso y por que) | Albert | Planner |

### Sprint 13: RAG + Feedback Loop (2 semanas)
> Knowledge base que crece con uso.

| ID | Tarea | Asignado | Modulo |
|----|-------|:--------:|--------|
| S13-01 | Template library: 10+ bots de referencia por industria (insurance, healthcare, finance, general) | Albert | Planner |
| S13-02 | Vector DB: Chroma embebido en Studio, indexing de templates al instalar | Luis | Planner |
| S13-03 | RAG pipeline: embed goal → search top-3 → include in context | Luis | Planner |
| S13-04 | Learn from success: guardar bots refinados como templates anonimizados | Luis | Planner |
| S13-05 | Telemetria: % planes sin cambios, % nodos correctos, tiempo refinamiento | Luis | Planner |
| S13-06 | A/B testing de prompts: probar versiones, medir calidad, seleccionar mejor | Albert + Luis | Planner |
| S13-07 | Community templates: opt-in para contribuir al knowledge base global (marketplace) | Luis | Planner |
| S13-08 | **Studio UI: Template browser** en AI Planner (ver y usar templates antes de generar) | Albert | Planner |
...
```
Sprint 0   ██ Estabilizacion                    (1 sem)  ✅ CERRADO
Sprint 1   ████ Providers + Azure Base          (2 sem)  ✅ CERRADO (providers)
Sprint 2   ████ RBAC + Contratos + CP UI        (2 sem)  🔄 EN PROGRESO
Sprint 3   ████ Licencias + Billing Real        (2 sem)
Sprint 4   ████ Partners + Marketplace          (2 sem)
Sprint 5   ████ Marketing Engine                (2 sem)
Sprint 6   ████ Support System                  (2 sem)
Sprint 7   ████ Orch UI + Operations            (2 sem)
Sprint 8   ████ Testing + Hardening             (2 sem)
Sprint 9   ████ Deploy + Certification          (2 sem)
Sprint 10  ████ Planner: Catalog + Enrichment   (2 sem)
Sprint 11  ████ Planner: Quality Rules (Rust)   (2 sem)
Sprint 12  ██████ Planner: MCP + ReAct Loop     (3 sem)
Sprint 13  ████ Planner: RAG + Feedback         (2 sem)
──────────────────────────────────────────────────────────
Total: 14 sprints, ~26 semanas, ~140 tareas

Workstreams paralelos: 5 Star Docu, Azure Deploy
```

---

*Plan maestro preparado por Albert - (c) 2026 Skuld, LLC*
