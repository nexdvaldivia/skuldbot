...
export const nodeTemplates: NodeTemplate[] = [
  ...
  // --- Cross-Provider Transfer ---
  {
    ...
    configSchema: [
      { name: "source_provider", label: "Source Provider", type: "text", required: false, description: "Optional. Leave empty to use connected Storage Provider." },
      { name: "source_path", label: "Source Path", type: "text", required: true, supportsExpressions: true, description: "Path/prefix in source provider (relative when using connected provider)." },
      { name: "dest_provider", label: "Destination Provider", type: "text", required: false, description: "Optional. Leave empty to use connected Storage Provider." },
      { name: "dest_path", label: "Destination Path", type: "text", required: true, supportsExpressions: true, description: "Path/prefix in destination provider (relative when using connected provider)." },
  ...
  // Note: Connect a "Storage Provider" node to use cloud storage instead of local filesystem
  {
    ...
    configSchema: [
      { name: "source", label: "Source File / Key", type: "text", required: true, supportsExpressions: true, description: "Relative source for connected Storage Provider. Absolute path allowed for local provider." },
  ...
  },
  {
    ...
    configSchema: [
      { name: "destination", label: "Destination File / Key", type: "text", required: true, supportsExpressions: true, description: "Relative destination for connected Storage Provider. Absolute path allowed for local provider." },
  ...
  },
  {
    ...
    configSchema: [
      { name: "target", label: "Target File / Key", type: "text", required: true, supportsExpressions: true },
  ...
  },
  {
    ...
    defaultConfig: {},
    configSchema: [
      { name: "target", label: "Target Folder / Prefix", type: "text", required: true, supportsExpressions: true },
    ],
    outputSchema: [
  ...
  },
  {
    ...
    icon: "FolderOpen",
    defaultConfig: { recursive: false, max_items: 1000, fail_on_empty: true },
    configSchema: [
      {
        name: "source",
        label: "Source Folder / Prefix",
        type: "text",
        supportsExpressions: true,
        description:
          "Relative source for connected Storage Provider. If empty, uses provider prefix. For local-only use, absolute path is allowed.",
      },
      { name: "pattern", label: "Pattern", type: "text", placeholder: "*.csv", description: "Glob pattern to filter files" },
      {
        name: "name_regex",
        label: "Name Regex (optional)",
        type: "text",
        supportsExpressions: true,
        placeholder: "^(?!~\\$).*\\.pdf$",
        description: "Python regex applied to the file name (basename) after pattern matching.",
      },
      ...
      { name: "max_items", label: "Max Items", type: "number", default: 1000, description: "Limit results (for large directories)" },
      {
        ...
        type: "boolean",
        default: true,
        description: "Fail node execution when no files match the source and pattern",
      },
    ],
    outputSchema: [
  ...
  },
  {
    ...
    defaultConfig: {},
    configSchema: [
      { name: "source", label: "Source File / Key", type: "text", required: true, supportsExpressions: true },
    ],
    outputSchema: [
  ...
  },
  {
    ...
    defaultConfig: {},
    configSchema: [
      { name: "source", label: "Source Folder / Prefix", type: "text", required: true, supportsExpressions: true },
  ...
  },
  {
    ...
    configSchema: [
      { name: "source", label: "Source File / Key", type: "text", required: true, supportsExpressions: true, description: "Requires a cloud Storage Provider connection" },
  ...
  // Connect AI Model node to configure the LLM provider
  {
    ...
    icon: "BrainCircuit",
    defaultConfig: { max_iterations: 10, execution_mode: "single" },
    configSchema: [
      { name: "name", label: "Agent Name", type: "text", placeholder: "My Agent" },
      {
        ...
        options: [
          ...
          { value: "per_item", label: "Per Item (Batch)" },
        ],
      },
      {
        name: "items",
        ...
        type: "text",
        supportsExpressions: true,
        visibleWhen: { field: "execution_mode", value: "per_item" },
        placeholder: "${node:upstream-map|results}",
        description: "Collection to process in per-item mode.",
      },
      {
        name: "item_var",
        label: "Item Variable",
        ...
        visibleWhen: { field: "execution_mode", value: "per_item" },
        placeholder: "item",
        description: "Variable exposed for each item (example: ${item.text}).",
      },
...
# }`
  ...
  // --- MFA / Authentication ---
  {
    ...
    category: "security",
    label: "TOTP Generate",
    description: "Generate a TOTP code for MFA authentication. The bot generates the same 6-digit code as the authenticator app on your phone.",
    icon: "ShieldCheck",
    defaultConfig: {
    ...
    configSchema: [
      {
        ...
        description: "Name of the TOTP shared secret stored in the connected KeyVault.",
      },
      {
        name: "mfa_type",
        ...
        options: [
          ...
          { value: "duo", label: "Duo Security" },
          { value: "okta_verify", label: "Okta Verify" },
          { value: "custom", label: "Custom TOTP" },
        ],
        description: "Which authenticator app the target system uses.",
      },
      {
        name: "digits",
        label: "Code Digits",
        type: "number",
        default: 6,
        description: "Only change for non-standard TOTP configurations.",
        visibleWhen: { field: "mfa_type", value: "custom" },
      },
      {
        name: "period",
    ...
    outputSchema: [
      { name: "code", type: "string", description: "The generated TOTP code (e.g., 847293)" },
      { name: "expiresIn", type: "number", description: "Seconds until this code expires" },
    ],
  },

  // --- Vault Providers (Enterprise Secrets Management) ---
  {
    type: "secrets.orchestrator_vault",
    category: "security",
    label: "Orchestrator Vault",
    description: "Connect to the Skuld Orchestrator vault. Secrets are managed centrally and resolved at runtime. This is the default vault for production bots.",
    icon: "KeyRound",
    defaultConfig: { auth_method: "runner_token" },
    configSchema: [
      { name: "orchestrator_url", label: "Orchestrator URL", type: "text", required: true, placeholder: "https://orchestrator.company.com", supportsExpressions: true, description: "URL of the Skuld Orchestrator. Can be set via SKULDBOT_ORCHESTRATOR_URL env var." },
      { name: "auth_method", label: "Authentication", type: "select", default: "runner_token", options: [
        { value: "runner_token", label: "Runner Token (from SKULDBOT_ORCHESTRATOR_TOKEN env)" },
        { value: "api_key", label: "API Key" },
      ]},
      { name: "api_key", label: "API Key", type: "password", secret: true, placeholder: "${vault.orchestrator_api_key}", supportsExpressions: true, description: "API key for Orchestrator authentication. Use ${vault.name} to resolve from local vault.", visibleWhen: { field: "auth_method", value: "api_key" } },
      { name: "secrets", label: "Secrets to Load", type: "textarea", placeholder: "erp-sap-mfa-secret\ndb-prod-password\napi-key-salesforce" },
    ],
    outputSchema: [
  ...
  },
  {
    ...
    configSchema: [
      ...
      { name: "use_managed_identity", label: "Use Managed Identity", type: "boolean", default: false, description: "Enable when running on Azure VMs/App Service. Azure authenticates automatically — no credentials needed." },
      { name: "tenant_id", label: "Tenant ID", type: "text", placeholder: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", description: "Azure AD Tenant ID. Required when not using Managed Identity.", visibleWhen: { field: "use_managed_identity", value: "false" } },
      { name: "client_id", label: "Client ID", type: "text", placeholder: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", description: "Application (client) ID of the Service Principal.", visibleWhen: { field: "use_managed_identity", value: "false" } },
      { name: "client_secret", label: "Client Secret", type: "password", secret: true, placeholder: "${vault.azure_client_secret}", supportsExpressions: true, description: "Service Principal secret. Use ${vault.name} to resolve from local vault — never hardcode.", visibleWhen: { field: "use_managed_identity", value: "false" } },
  ...
  },
  {
    ...
    configSchema: [
      ...
      { name: "use_iam_role", label: "Use IAM Role", type: "boolean", default: true, description: "Enable when running on AWS EC2/ECS/Lambda. Disables access keys — AWS authenticates via instance role." },
      { name: "access_key_id", label: "Access Key ID", type: "text", placeholder: "AKIAIOSFODNN7EXAMPLE", visibleWhen: { field: "use_iam_role", value: "false" } },
      { name: "secret_access_key", label: "Secret Access Key", type: "password", secret: true, placeholder: "${vault.aws_secret_key}", supportsExpressions: true, description: "Use ${vault.name} to resolve from local vault — never hardcode.", visibleWhen: { field: "use_iam_role", value: "false" } },
  ...
  },
  {
    ...
    configSchema: [
      ...
      { name: "token", label: "Vault Token", type: "password", secret: true, placeholder: "${vault.hashicorp_token}", supportsExpressions: true, description: "Vault token. Can also be set via VAULT_TOKEN env var. Use ${vault.name} to resolve from local vault.", visibleWhen: { field: "auth_method", value: "token" } },
      { name: "role_id", label: "Role ID", type: "text", placeholder: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", description: "AppRole Role ID. Can also be set via VAULT_ROLE_ID env var.", visibleWhen: { field: "auth_method", value: "approle" } },
      { name: "secret_id", label: "Secret ID", type: "password", secret: true, placeholder: "${vault.hashicorp_secret_id}", supportsExpressions: true, description: "AppRole Secret ID. Use ${vault.name} to resolve from local vault.", visibleWhen: { field: "auth_method", value: "approle" } },
  ...
  {
    type: "secrets.gcp_secret_manager",
    category: "security",
    label: "GCP Secret Manager",
    description: "Connect to Google Cloud Secret Manager (credentials from BotRunner env or Service Account)",
    icon: "KeyRound",
    defaultConfig: { project_id: "", use_default_credentials: true },
    configSchema: [
      { name: "project_id", label: "GCP Project ID", type: "text", required: true, placeholder: "my-project-123", supportsExpressions: true },
      { name: "use_default_credentials", label: "Use Application Default Credentials", type: "boolean", default: true, description: "Uses GOOGLE_APPLICATION_CREDENTIALS env or GCE metadata. Disable to provide a service account key." },
      { name: "service_account_key", label: "Service Account Key (JSON)", type: "textarea", placeholder: '{"type":"service_account","project_id":"..."}', secret: true, visibleWhen: { field: "use_default_credentials", value: "false" } },
      { name: "secrets", label: "Secrets to Load", type: "textarea", placeholder: "projects/my-project/secrets/db-password/versions/latest\nprojects/my-project/secrets/api-key/versions/latest" },
    ],
    outputSchema: [
      { name: "loaded", type: "number", description: "Number of secrets loaded" },
      { name: "secretNames", type: "array", description: "Names of loaded secrets" },
    ...
    configSchema: [
      {
        ...
        required: true,
        supportsExpressions: true,
        description: "Relative file/key. Base location comes from connected Storage Provider.",
      },
      { name: "delimiter", label: "Delimiter", type: "text", default: "," },
      { name: "encoding", label: "Encoding", type: "select", default: "utf-8", options: [
        { value: "utf-8", label: "UTF-8" },
  ...
  {
    type: "data.tap.hubspot",
    category: "data",
    label: "Extract HubSpot",
    description: "Extract data from HubSpot REST API",
    icon: "Globe",
    defaultConfig: { method: "GET", auth_type: "bearer", data_path: "$.results" },
    configSchema: [
      { name: "url", label: "HubSpot API URL", type: "text", required: true, supportsExpressions: true, placeholder: "https://api.hubapi.com/crm/v3/objects/contacts?limit=100" },
      { name: "method", label: "HTTP Method", type: "select", default: "GET", options: [
        { value: "GET", label: "GET" },
        { value: "POST", label: "POST" },
      ]},
      { name: "headers", label: "Headers (JSON)", type: "textarea", placeholder: '{"Accept": "application/json"}' },
      { name: "body", label: "Request Body (JSON)", type: "textarea" },
      { name: "auth_type", label: "Auth Type", type: "select", default: "bearer", options: [
        { value: "none", label: "None" },
        { value: "bearer", label: "Bearer Token" },
        { value: "api_key", label: "API Key" },
      ]},
      { name: "auth_value", label: "HubSpot Token", type: "password", required: true, supportsExpressions: true },
      { name: "pagination_type", label: "Pagination", type: "select", options: [
        { value: "none", label: "None" },
        { value: "offset", label: "Offset" },
        { value: "page", label: "Page Number" },
      ...
      { name: "pagination_param", label: "Pagination Param", type: "text", placeholder: "after" },
      { name: "data_path", label: "Data Path (JSONPath)", type: "text", default: "$.results", placeholder: "$.results" },
      { name: "limit", label: "Max Records", type: "number" },
    ],
    outputSchema: [
      { name: "records", type: "array", description: "HubSpot records" },
      { name: "columns", type: "array", description: "Field names" },
      { name: "recordCount", type: "number", description: "Number of records" },
    ],
  },
  {
    type: "data.tap.zendesk",
    category: "data",
    label: "Extract Zendesk",
    description: "Extract data from Zendesk REST API",
    icon: "Globe",
    defaultConfig: { method: "GET", auth_type: "bearer", data_path: "$.tickets", pagination_type: "page", pagination_param: "page" },
    configSchema: [
      { name: "url", label: "Zendesk API URL", type: "text", required: true, supportsExpressions: true, placeholder: "https://your-subdomain.zendesk.com/api/v2/tickets.json" },
      { name: "method", label: "HTTP Method", type: "select", default: "GET", options: [
        { value: "GET", label: "GET" },
        { value: "POST", label: "POST" },
      ]},
      ...
      { name: "auth_value", label: "Zendesk Token", type: "password", required: true, supportsExpressions: true },
      { name: "pagination_type", label: "Pagination", type: "select", default: "page", options: [
        { value: "none", label: "None" },
        { value: "offset", label: "Offset" },
        { value: "page", label: "Page Number" },
        { value: "cursor", label: "Cursor" },
      ]},
      { name: "pagination_param", label: "Pagination Param", type: "text", default: "page", placeholder: "page" },
      { name: "data_path", label: "Data Path (JSONPath)", type: "text", default: "$.tickets", placeholder: "$.tickets" },
      { name: "limit", label: "Max Records", type: "number" },
    ],
    outputSchema: [
      { name: "records", type: "array", description: "Zendesk records" },
      { name: "columns", type: "array", description: "Field names" },
      { name: "recordCount", type: "number", description: "Number of records" },
    ],
  },
  {
    ...
    icon: "Globe",
    defaultConfig: { method: "GET", auth_type: "bearer", data_path: "$.issues", pagination_type: "offset", pagination_param: "startAt" },
    configSchema: [
      { name: "url", label: "Jira API URL", type: "text", required: true, supportsExpressions: true, placeholder: "https://your-domain.atlassian.net/rest/api/3/search?maxResults=100" },
      { name: "method", label: "HTTP Method", type: "select", default: "GET", options: [
        { value: "GET", label: "GET" },
        { value: "POST", label: "POST" },
      ]},
      ...
      { name: "auth_value", label: "Jira Token", type: "password", required: true, supportsExpressions: true },
      { name: "pagination_type", label: "Pagination", type: "select", default: "offset", options: [
        { value: "none", label: "None" },
        { value: "offset", label: "Offset" },
        { value: "page", label: "Page Number" },
        { value: "cursor", label: "Cursor" },
      ]},
      { name: "pagination_param", label: "Pagination Param", type: "text", default: "startAt", placeholder: "startAt" },
      { name: "data_path", label: "Data Path (JSONPath)", type: "text", default: "$.issues", placeholder: "$.issues" },
      { name: "limit", label: "Max Records", type: "number" },
    ],
    outputSchema: [
      { name: "records", type: "array", description: "Jira records" },
      { name: "columns", type: "array", description: "Field names" },
      { name: "recordCount", type: "number", description: "Number of records" },
    ],
  },
  {
    type: "data.tap.github",
    category: "data",
    label: "Extract GitHub",
    description: "Extract data from GitHub REST API",
    icon: "Globe",
    defaultConfig: { method: "GET", auth_type: "bearer", pagination_type: "page", pagination_param: "page" },
    configSchema: [
      { name: "url", label: "GitHub API URL", type: "text", required: true, supportsExpressions: true, placeholder: "https://api.github.com/repos/owner/repo/issues?per_page=100" },
      { name: "method", label: "HTTP Method", type: "select", default: "GET", options: [
        { value: "GET", label: "GET" },
        { value: "POST", label: "POST" },
      ]},
      { name: "headers", label: "Headers (JSON)", type: "textarea", placeholder: '{"Accept": "application/vnd.github+json"}' },
      { name: "body", label: "Request Body (JSON)", type: "textarea" },
      { name: "auth_type", label: "Auth Type", type: "select", default: "bearer", options: [
        { value: "none", label: "None" },
        { value: "bearer", label: "Bearer Token" },
      ...
      { name: "auth_value", label: "GitHub Token", type: "password", required: true, supportsExpressions: true },
      { name: "pagination_type", label: "Pagination", type: "select", default: "page", options: [
        { value: "none", label: "None" },
        { value: "offset", label: "Offset" },
        { value: "page", label: "Page Number" },
      ...
      { name: "pagination_param", label: "Pagination Param", type: "text", default: "page", placeholder: "page" },
      { name: "data_path", label: "Data Path (JSONPath)", type: "text", placeholder: "Leave empty for top-level arrays" },
      { name: "limit", label: "Max Records", type: "number" },
    ],
    outputSchema: [
      { name: "records", type: "array", description: "GitHub records" },
      { name: "columns", type: "array", description: "Field names" },
      { name: "recordCount", type: "number", description: "Number of records" },
    ],
  },
  {
    type: "data.tap.quickbooks",
    category: "data",
    label: "Extract QuickBooks",
    description: "Extract data from QuickBooks Online REST API",
    icon: "Globe",
    defaultConfig: { method: "GET", auth_type: "bearer", data_path: "$.QueryResponse" },
    configSchema: [
      { name: "url", label: "QuickBooks API URL", type: "text", required: true, supportsExpressions: true, placeholder: "https://quickbooks.api.intuit.com/v3/company/{companyId}/query?query=SELECT%20*%20FROM%20Invoice" },
      { name: "method", label: "HTTP Method", type: "select", default: "GET", options: [
        { value: "GET", label: "GET" },
        { value: "POST", label: "POST" },
      ]},
      ...
      { name: "auth_value", label: "QuickBooks Token", type: "password", required: true, supportsExpressions: true },
      { name: "pagination_type", label: "Pagination", type: "select", options: [
        { value: "none", label: "None" },
        { value: "offset", label: "Offset" },
        { value: "page", label: "Page Number" },
      ...
      { name: "pagination_param", label: "Pagination Param", type: "text", placeholder: "startposition or page" },
      { name: "data_path", label: "Data Path (JSONPath)", type: "text", default: "$.QueryResponse", placeholder: "$.QueryResponse" },
      { name: "limit", label: "Max Records", type: "number" },
    ],
    outputSchema: [
      { name: "records", type: "array", description: "QuickBooks records" },
      { name: "columns", type: "array", description: "Field names" },
      { name: "recordCount", type: "number", description: "Number of records" },
    ],
  },
  {
    type: "data.tap.dynamics365",
    category: "data",
    label: "Extract Dynamics 365",
    description: "Extract data from Dynamics 365 Web API",
    icon: "Globe",
    defaultConfig: { method: "GET", auth_type: "bearer", data_path: "$.value" },
    configSchema: [
      { name: "url", label: "Dynamics 365 API URL", type: "text", required: true, supportsExpressions: true, placeholder: "https://org.crm.dynamics.com/api/data/v9.2/accounts?$top=100" },
      { name: "method", label: "HTTP Method", type: "select", default: "GET", options: [
        { value: "GET", label: "GET" },
        { value: "POST", label: "POST" },
      ]},
      ...
      { name: "auth_value", label: "Dynamics 365 Token", type: "password", required: true, supportsExpressions: true },
      { name: "pagination_type", label: "Pagination", type: "select", options: [
        { value: "none", label: "None" },
        { value: "offset", label: "Offset" },
        { value: "page", label: "Page Number" },
      ...
      { name: "pagination_param", label: "Pagination Param", type: "text", placeholder: "$skiptoken or page" },
      { name: "data_path", label: "Data Path (JSONPath)", type: "text", default: "$.value", placeholder: "$.value" },
      { name: "limit", label: "Max Records", type: "number" },
    ],
    outputSchema: [
      { name: "records", type: "array", description: "Dynamics 365 records" },
      { name: "columns", type: "array", description: "Field names" },
      { name: "recordCount", type: "number", description: "Number of records" },
    ],
  },
  ...
  // --- FILE TARGETS ---
  {
    ...
    icon: "FileSpreadsheet",
    defaultConfig: { target_path: "output.csv", delimiter: ",", encoding: "utf-8" },
    configSchema: [
      {
        name: "target_path",
        label: "Target File / Key",
        type: "text",
        required: true,
        supportsExpressions: true,
        description: "Relative file/key. Base location comes from connected Storage Provider.",
      ...
      {
        name: "columns",
        label: "CSV Column Constructor (optional)",
        type: "csv-column-builder",
        description: "Define column headers, types, default values and field mapping for the output CSV.",
      },
      { name: "delimiter", label: "Delimiter", type: "text", default: "," },
      { name: "encoding", label: "Encoding", type: "select", default: "utf-8", options: [
        { value: "utf-8", label: "UTF-8" },
  ...
  },
  {
    ...
    icon: "FileSpreadsheet",
    defaultConfig: { target_path: "output.xlsx", sheet: "Sheet1" },
    configSchema: [
      {
        name: "target_path",
        label: "Target File / Key",
  ...
  },
  {
    ...
    category: "data",
    label: "Write QBO",
    description: "Write data to QuickBooks QBO (OFX)",
    icon: "FileText",
    defaultConfig: {
      target_path: "output.qbo",
    ...
    configSchema: [
      ...
      { name: "account_id", label: "Account ID", type: "text", default: "000000000", supportsExpressions: true },
      { name: "bank_id", label: "Bank ID", type: "text", default: "000000000", supportsExpressions: true },
      { name: "currency", label: "Currency", type: "text", default: "USD", supportsExpressions: true },
      { name: "date_field", label: "Date Field", type: "text", default: "date" },
      { name: "amount_field", label: "Amount Field", type: "text", default: "amount" },
      { name: "payee_field", label: "Payee Field", type: "text", default: "name" },
      { name: "memo_field", label: "Memo Field", type: "text", default: "memo" },
      { name: "fitid_field", label: "FITID Field", type: "text", default: "fitid" },
      { name: "type_field", label: "Type Field", type: "text", default: "type" },
      { name: "date_format", label: "Date Format (optional)", type: "text", placeholder: "%Y-%m-%d" },
      { name: "intu_bid", label: "INTU.BID", type: "text", default: "3000", supportsExpressions: true },
      {
        name: "encoding",
        label: "Encoding",
        type: "select",
    ...
    outputSchema: [
      { name: "insertedCount", type: "number", description: "Transactions written" },
      { name: "errorCount", type: "number", description: "Transactions skipped due to errors" },
    ],
  },
