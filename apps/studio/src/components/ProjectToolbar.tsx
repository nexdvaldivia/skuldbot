...
export default function ProjectToolbar() {
  ...
  const handleCompile = async () => {
    ...
    try {
      ...
      const errorMessage = normalizeInvokeErrorMessage(error);
      logs.error("Compilation error", errorMessage);
  ...
  const handleRun = async (formData?: Record<string, any>) => {
    ...
    if (debugState === "running" || debugState === "paused") {
      toast.warning("Debug active", "Stop or finish debugging before running the bot");
      return;
    }
    ...
    try {
      ...
      const errorMessage = normalizeInvokeErrorMessage(error);
      logs.error("Execution error", errorMessage);
    ...
    <header className="h-12 bg-white border-b border-slate-200 flex items-center justify-between px-4 flex-shrink-0">
      ...
      <div className="flex items-center gap-1">
        ...
        <Button
          ...
          size="sm"
          onClick={() => handleRun()}
          disabled={isRunning || debugState === "running" || debugState === "paused" || !hasNodes}
          title="Run"
          className="text-slate-500 hover:text-slate-700"
        >
          {isRunning ? (
