...
export default function SettingsPanel() {
  ...
  const [edgeAnchorTolerance, setEdgeAnchorToleranceState] = useState(8);
  ...
  useEffect(() => {
    setEdgeAnchorToleranceState(getEdgeRouteAnchorTolerance());
  }, []);
  ...
  const handleEdgeToleranceChange = (value: string) => {
    const nextTolerance = parseInt(value, 10);
    setEdgeAnchorToleranceState(nextTolerance);
    setEdgeRouteAnchorTolerance(nextTolerance);
  };
          ...
          <div className="bg-white rounded-xl border border-slate-200">
            <div className="px-5 py-4 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-medium text-slate-800">Edge Routing</h2>
              </div>
              <p className="text-sm text-slate-500 mt-1">
                Control how aggressively manual edge segments merge when aligned
              </p>
            </div>
            <div className="p-5 space-y-2">
              <Label htmlFor="edge-anchor-tolerance" className="text-sm font-medium text-slate-700">
                Segment Merge Tolerance
              </Label>
              <Select
                value={String(edgeAnchorTolerance)}
                onValueChange={handleEdgeToleranceChange}
              >
                <SelectTrigger id="edge-anchor-tolerance" className="h-10 border-slate-200">
                  <SelectValue placeholder="Select tolerance" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="4">4 px (precise)</SelectItem>
                  <SelectItem value="8">8 px (recommended)</SelectItem>
                  <SelectItem value="12">12 px (relaxed)</SelectItem>
                  <SelectItem value="16">16 px (aggressive)</SelectItem>
                </SelectContent>
              </Select>
              <p className="text-xs text-slate-400">
                Higher tolerance snaps center points faster to source/target lines and fuses segments sooner. Applies immediately.
              </p>
            </div>
          </div>
        </div>
