      ...
      <div className="flex items-center gap-1 px-3 py-2 border-b">
        ...
        <button
          ...
          className={cn(
            ...
            !isPaused ? 'text-slate-300 cursor-not-allowed' : 'hover:bg-blue-100 text-blue-600',
          )}
          title="Step Over (F10)"
        >
        ...
        {pauseRequested && isRunning && (
          <span className="text-xs text-amber-600 ml-1">(next node boundary)</span>
        )}

        {/* Session info */}
        {sessionState && (
          <span className="text-xs text-slate-400 ml-2">
            Node {sessionState.executionOrder.indexOf(sessionState.currentNodeId || '') + 1}/
            {sessionState.executionOrder.length}
          </span>
        )}
        ...
        {breakpoints.size === 0 ? (
          ...
          <div className="space-y-1 max-h-24 overflow-y-auto">
            {Array.from(breakpoints).map((nodeId) => (
              <div key={nodeId} className="flex items-center gap-2 text-xs text-slate-600 py-0.5">
              ...
              .map(([nodeId, exec]) => (
                <div key={nodeId}>
                  ...
                  >
                    ...
                    {exec.label || nodeId}
                    <span className="text-slate-400 text-[10px] ml-1">({exec.status})</span>
                  </button>
            ...
            <div className="px-3 py-2 border-t">
              ...
              <div className="space-y-0.5 max-h-48 overflow-y-auto">
                {executionHistory.map((exec, idx) => {
                  ...
                  return (
                    <div key={`${exec.nodeId}-${idx}`} className="flex items-start gap-1.5">
                      {/* Timeline connector */}
                      ...
                      >
                        ...
                        {exec.error && (
                          <div
                            className="text-[10px] text-red-600 mt-0.5 truncate"
                            title={exec.error}
                          >
                            {exec.error}
                          </div>
      ...
      {sessionState?.pausedAtBreakpoint && (
        <div className="px-3 py-2 bg-yellow-50 border-t border-yellow-200">
          <div className="flex items-center gap-2 text-xs text-yellow-700">
            <Circle className="w-2 h-2 fill-red-500 text-red-500" />
            <span>
              Paused at breakpoint: <strong>{sessionState.currentNodeId}</strong>
            </span>
          </div>
        </div>
      )}
