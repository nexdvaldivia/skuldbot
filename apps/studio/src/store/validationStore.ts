...
const validateNodes = (nodes: FlowNode[], edges: FlowEdge[]): ValidationIssue[] => {
  ...
  const isStorageConnectionTargetType = (type: string) =>
    type === "storage.transfer" ||
    type === "storage.sync" ||
    isStorageModeNodeType(type);
  const hasStorageProviderConnection = (nodeId: string) =>
    edges.some((e) => {
      if (e.target !== nodeId) return false;
      const sourceNode = nodes.find((n) => n.id === e.source);
      if (!sourceNode || sourceNode.data.nodeType !== "storage.provider") return false;
      const targetNode = nodes.find((n) => n.id === nodeId);
      const isStorageTarget = targetNode
        ? isStorageConnectionTargetType(targetNode.data.nodeType)
        : false;

      // Primary path: explicit storage connection handles.
      if (e.targetHandle === "connection" && (e.data?.edgeType === "connection" || e.sourceHandle === "connection-out")) {
        return true;
      }

      // Backward compatibility for legacy edges.
  ...
  const isAbsolutePathLike = (value: unknown) => {
    if (typeof value !== "string") return false;
    const candidate = value.trim();
    if (!candidate) return false;
    return (
  ...
  const isExpressionLike = (value: unknown) => {
    if (typeof value !== "string") return false;
    const candidate = value.trim();
    if (!candidate) return false;
  ...
  const getNodeStorageMode = (
    ...
    isStorageModeNodeType(nodeType)
      ? normalizeStorageMode(
          config.storage_mode,
          hasStorageProviderConnection(nodeId)
            ? "provider_relative_path"
            : "absolute_path_only"
        )
      : DEFAULT_STORAGE_MODE;

  const getStoragePathValue = (
    field: string,
    config: Record<string, any>
  ): unknown => {
    if (field === "source") {
      return config.source ?? config.path;
    }
    if (field === "destination") {
      return config.destination ?? config.path ?? config.target;
    }
    if (field === "target") {
      return config.target ?? config.path;
    }
    if (field === "target_path") {
      return config.target_path ?? config.path;
    }
    return config[field];
  };
  ...
  const isFieldVisible = (
    ...
    const dependentFieldValue = config[field.visibleWhen.field];
    const expectedValue = field.visibleWhen.value;
    if (Array.isArray(expectedValue)) {
      return expectedValue.includes(dependentFieldValue);
    }
    return dependentFieldValue === expectedValue;
  };

  const getConfigValueWithAliases = (
    nodeType: string,
    fieldName: string,
    config: Record<string, any>
  ) => {
    ...
    if (fieldName === "destination" && nodeType === "files.write") {
      return config.destination ?? config.path ?? config.target;
    }
    ...
    if (fieldName === "target" && (nodeType === "files.delete" || nodeType === "files.create_folder")) {
      return config.target ?? config.path;
    }

    if (fieldName === "source_path" && (nodeType === "storage.transfer" || nodeType === "storage.sync")) {
      return config.source_path ?? config.source;
    }

    if (fieldName === "dest_path" && (nodeType === "storage.transfer" || nodeType === "storage.sync")) {
      return config.dest_path ?? config.destination ?? config.dest;
    }

    if (fieldName === "dest_provider" && (nodeType === "storage.transfer" || nodeType === "storage.sync")) {
      return config.dest_provider ?? config.destination_provider;
    }
  ...
  nodes.forEach((node) => {
    ...
    switch (nodeType) {
      ...
      case "storage.sync": {
        const sourcePathValue =
          typeof config.source_path === "string" ? config.source_path.trim() : config.source_path;
        const destPathValue =
          typeof config.dest_path === "string" ? config.dest_path.trim() : config.dest_path;
        const legacySourceValue =
          typeof config.source === "string" ? config.source.trim() : config.source;
        const legacyDestinationValue =
          typeof config.destination === "string" ? config.destination.trim() : config.destination;
        const legacyDestValue = typeof config.dest === "string" ? config.dest.trim() : config.dest;

        if (isMissingValue(sourcePathValue) && isMissingValue(legacySourceValue)) {
          issues.push({
            id: `${node.id}-source-path`,
            nodeId: node.id,
            severity: "error",
        ...
        if (
          isMissingValue(destPathValue) &&
          isMissingValue(legacyDestinationValue) &&
          isMissingValue(legacyDestValue)
        ) {
          issues.push({
            id: `${node.id}-dest-path`,
            nodeId: node.id,
            severity: "error",
        ...
        const hasStorageConnection = hasStorageProviderConnection(node.id);
        const sourceProviderValue =
          typeof config.source_provider === "string"
            ? config.source_provider.trim()
            : config.source_provider;
        const destProviderValue =
          typeof config.dest_provider === "string"
            ? config.dest_provider.trim()
            : config.dest_provider;
        const legacyDestProviderValue =
          typeof config.destination_provider === "string"
            ? config.destination_provider.trim()
            : config.destination_provider;

        const hasSourceProvider = !isMissingValue(sourceProviderValue);
        const hasDestProvider =
          !isMissingValue(destProviderValue) ||
          !isMissingValue(legacyDestProviderValue);

        if (!hasStorageConnection && !hasSourceProvider) {
          issues.push({
            id: `${node.id}-source-provider`,
            nodeId: node.id,
            severity: "error",
            message: "Source Provider is required when no Storage Provider node is connected",
            field: "source_provider",
          });
        }
        if (!hasStorageConnection && !hasDestProvider) {
          issues.push({
            id: `${node.id}-dest-provider`,
            nodeId: node.id,
            severity: "error",
            message: "Destination Provider is required when no Storage Provider node is connected",
            field: "dest_provider",
          });
        }
        break;
    ...
    if (isStorageModeNodeType(nodeType)) {
      const storageMode = getNodeStorageMode(nodeType, config, node.id);
      const hasStorageConnection = hasStorageProviderConnection(node.id);
      const pathFields = getStoragePathFieldsForNodeType(nodeType);
      const pathValues = pathFields.map((field) => getStoragePathValue(field, config));

      if (config.storage_mode && normalizeStorageMode(config.storage_mode) !== config.storage_mode) {
        issues.push({
          id: `${node.id}-storage-mode-invalid`,
          nodeId: node.id,
          severity: "warning",
          message: "Invalid storage_mode. Falling back to provider_relative_path.",
          field: "storage_mode",
        });
      }

      if (storageMode !== "absolute_path_only" && !hasStorageConnection) {
        issues.push({
          id: `${node.id}-storage-connection`,
          nodeId: node.id,
          severity: "error",
          message: `Connect a Storage Provider node to "${node.data.label}" or switch mode to absolute path`,
          field: "connection",
        });
      }

      if (nodeType === "files.presigned_url" && storageMode === "absolute_path_only") {
        issues.push({
          id: `${node.id}-storage-mode-presigned`,
          nodeId: node.id,
          severity: "error",
          message: "files.presigned_url requires provider mode (cannot run in absolute_path_only).",
          field: "storage_mode",
        });
      }

      if (storageMode === "provider_only") {
        pathFields.forEach((field, index) => {
          if (!isMissingValue(pathValues[index])) {
            issues.push({
              id: `${node.id}-${field}-ignored`,
              nodeId: node.id,
              severity: "warning",
              message: `${field} is ignored in provider_only mode.`,
              field,
            });
          }
        });
        ...
        pathFields.forEach((field, index) => {
          const value = pathValues[index];
          if (isMissingValue(value)) {
            issues.push({
              id: `${node.id}-${field}`,
              nodeId: node.id,
              severity: "error",
              message: `${field} is required for ${storageMode}`,
              field,
            });
            return;
          }

          if (storageMode === "absolute_path_only") {
            if (!isExpressionLike(value) && !isAbsolutePathLike(value)) {
              issues.push({
                id: `${node.id}-${field}-absolute`,
                nodeId: node.id,
                severity: "error",
                message: `${field} must be an absolute path in absolute_path_only mode.`,
                field,
              });
            }
          } else if (storageMode === "provider_relative_path") {
            if (!isExpressionLike(value) && isAbsolutePathLike(value)) {
              issues.push({
                id: `${node.id}-${field}-relative-warning`,
                nodeId: node.id,
                severity: "warning",
                message: `${field} should be relative in provider_relative_path mode.`,
                field,
              });
            }
          }
    ...
    if (AI_NODES_WITH_MODEL.has(nodeType)) {
      const hasModelConnection = edges.some(
        (e) =>
          ...
          (e.data?.edgeType === "model" || e.sourceHandle === "model-out")
      );
    ...
    if (isMs365Action(nodeType)) {
      const hasMS365Connection = edges.some(
        (e) =>
          ...
          (e.data?.edgeType === "connection" || e.sourceHandle === "connection-out")
      );
      if (!hasMS365Connection) {
        issues.push({
          id: `${node.id}-no-connection`,
    ...
    if (nodeType.startsWith("data.tap.")) {
      ...
      if (["sqlserver", "oracle", "postgres", "mysql", "db2", "snowflake"].includes(tapType)) {
        ...
        // Validated centrally via storage_mode rules.
      } else if (tapType === "s3") {
    ...
    if (nodeType.startsWith("data.target.")) {
      ...
      } else if (targetType === "csv" || targetType === "excel" || targetType === "qbo") {
        // Validated centrally via storage_mode rules.
    ...
    if (template?.configSchema?.length) {
      template.configSchema.forEach((field) => {
        ...
        if (
          isStorageModeNodeType(nodeType) &&
          getNodeStorageMode(nodeType, config, node.id) === "provider_only" &&
          isStoragePathField(nodeType, field.name)
        ) {
          return;
        }
        ...
        const resolvedValue = getConfigValueWithAliases(nodeType, field.name, config);
        if (hasDefault && (resolvedValue === undefined || resolvedValue === null)) return;
        if (isMissingValue(resolvedValue)) {
          issues.push({
