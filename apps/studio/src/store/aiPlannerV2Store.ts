...
function normalizeProjectName(goal: string): string {
  const trimmed = String(goal || "").trim();
  if (!trimmed) return "AI Planner Project";
  const cleaned = trimmed.replace(/[^\p{L}\p{N}\s\-_]/gu, " ").replace(/\s+/g, " ").trim();
  return cleaned.slice(0, 64) || "AI Planner Project";
}

function slugifyProjectName(name: string): string {
  return String(name || "ai-planner-project")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "")
    .slice(0, 64) || "ai-planner-project";
}
...
export const useAIPlannerV2Store = create<AIPlannerV2State>()(
  persist(
    (set, get) => ({
      ...
      // ============================================================
      // Apply as Project (Project + Bot scaffolding)
      // ============================================================

      applyAsProject: async () => {
        const { currentPlan } = get();
        const toast = useToastStore.getState();
        ...
        if (!currentPlan.validation.valid || !currentPlan.validation.compilable) {
          toast.error("Invalid Plan", "Fix validation errors before creating a project");
          return;
        }

        try {
          const selectedBaseDir = await open({
            directory: true,
            multiple: false,
            title: "Select folder where the new AI Planner project will be created",
          });

          if (!selectedBaseDir || typeof selectedBaseDir !== "string") {
            toast.info("Canceled", "Project creation canceled");
            return;
          }

          const projectName = normalizeProjectName(currentPlan.goal);
          const projectSlug = slugifyProjectName(projectName);
          const projectPath = `${selectedBaseDir}/${projectSlug}`;
          const botName = projectName;

          const projectStore = useProjectStore.getState();
          const tabsStore = useTabsStore.getState();
          const navigationStore = useNavigationStore.getState();

          await projectStore.createProject(projectPath, projectName, currentPlan.description);

          const afterCreate = useProjectStore.getState();
          if (afterCreate.projectPath !== projectPath) {
            throw new Error("Could not initialize project state after creation");
          }

          navigationStore.setView("project");

          const botId = await projectStore.createBot(
            botName,
            `Generated by AI Planner for goal: ${currentPlan.goal}`
          );

          if (!botId) {
            throw new Error("Could not create bot inside the new project");
          }

          await projectStore.openBot(botId);
          ...
          // Let BotEditor mount and then reuse existing planner apply event path.
          ...
          // Persist generated graph into bot.json.
          await new Promise((resolve) => setTimeout(resolve, 120));
          await useProjectStore.getState().saveBot(botId, true);

          toast.success(
            "Project + Bot Created",
            `Created ${projectName} with bot ${botName} and applied ${currentPlan.tasks.length} steps`
          );

          get().closePanel();
        } catch (error) {
          console.error("❌ Failed to apply plan as project:", error);
          toast.error("Create Project Failed", String(error));
        }
      },
