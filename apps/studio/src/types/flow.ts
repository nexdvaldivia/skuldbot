...
export interface BotDSL {
  ...
  ui?: {
    edgeRoutes?: Record<string, EdgeRouteControlPoint>;
  };
}

export interface EdgeRouteControlPoint {
  centerX: number;
  centerY: number;
  manual?: boolean;
}
...
export interface ConfigField {
  name: string;
  label: string;
  type: "text" | "number" | "boolean" | "select" | "textarea" | "password" | "form-builder" | "expression" | "validation-builder" | "protection-builder" | "node-select" | "bot-select" | "csv-column-builder";
  required?: boolean;
  default?: any;
  ...
  visibleWhen?: { field: string; value: string | string[] };  // Conditional visibility based on another field's value (use String() coercion for booleans)
  nodeType?: string;  // For "node-select" type: the node type to filter by (e.g., "storage.provider")
  browseMode?: "file" | "directory" | "none"; // Optional override for local picker button behavior
}
...
export interface FlowNodeData {
  ...
  // Persisted visual connections to config nodes (model/tools/memory/connection)
  connections?: {
    model?: string;
    tools?: string[];
    memory?: string;
...
export interface FlowEdgeData {
  ...
  // Optional manual routing control point for visual layout in Studio
  route?: EdgeRouteControlPoint;
}
