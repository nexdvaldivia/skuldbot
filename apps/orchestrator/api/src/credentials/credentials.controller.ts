...
import {
  getAllSecretsForCloud,
  getAutoGeneratedSecrets,
 ...
 * - GET    /credentials/secret-manifest    - Get deployment secret manifest
...
export class CredentialsController {
  ...
  /**
   * Get deployment secret manifest.
   *
   * Useful for bootstrap automation, IaC workflows, and security reviews.
   */
  @Get('secret-manifest')
  @RequirePermissions('credentials:read')
  async getSecretManifest(
    @Query('cloud') cloud = 'generic',
    @Query('includeOptional') includeOptional = 'true',
  ): Promise<{
    cloud: SupportedCloud;
    requiredSecretNames: string[];
    autoGeneratedSecretNames: string[];
    secrets: ReturnType<typeof getAllSecretsForCloud>;
  }> {
    const normalizedCloud = ['azure', 'aws', 'gcp', 'generic'].includes(cloud)
      ? (cloud as SupportedCloud)
      : 'generic';
    const withOptional = includeOptional.toLowerCase() !== 'false';
    const secrets = getAllSecretsForCloud(normalizedCloud, withOptional);

    return {
      cloud: normalizedCloud,
      requiredSecretNames: getRequiredSecretNames(),
      autoGeneratedSecretNames: getAutoGeneratedSecrets(withOptional).map((item) => item.name),
      secrets,
    };
  }
  ...
  async updateValue(
    ...
    return this.credentialsService.updateValue(tenantId, credentialId, user.id, dto);
  }
  ...
  async revoke(
    ...
    return this.credentialsService.revoke(tenantId, credentialId, user.id, reason);
  }
  ...
  @Get('vaults')
  @RequirePermissions('credentials:admin')
  async listVaultConnections(@TenantId() tenantId: string): Promise<VaultConnectionDto[]> {
