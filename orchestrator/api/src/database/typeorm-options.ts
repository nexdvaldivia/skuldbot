...
export function buildTypeOrmOptions(env: NodeJS.ProcessEnv): DataSourceOptions {
  ...
  const synchronize = isTrue(env.DATABASE_SYNCHRONIZE) && isLocalRuntime(nodeEnv);

  return {
    type: 'postgres',
    host: requireEnv(env, 'DATABASE_HOST'),
    port: Number.parseInt(requireEnv(env, 'DATABASE_PORT'), 10),
    username: requireEnv(env, 'DATABASE_USER'),
    password: requireEnv(env, 'DATABASE_PASSWORD', { allowEmpty: true }),
    database: requireEnv(env, 'DATABASE_NAME'),
    entities: databaseEntities,
    migrations: [path.join(__dirname, 'migrations', '*.{ts,js}')],
    synchronize,
    logging: isTrue(env.DATABASE_LOGGING),
  };
}
